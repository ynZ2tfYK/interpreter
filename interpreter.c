...
static void load_plugins() {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        const Plugin *plugin = init_plugin(); // Changed to const
...
// Executes the parsed expression or assignment, returns 0 on success, 1 on error
...
int main() {
    ...
    while (1) {
        ...
        // Check if input ends with .script
        ...
        if (len >= 7 && strcmp(code + len - 7, ".script") == 0) {
            ...
            // Process as a direct expression
            if (!parse(&parser, code)) {
                printf("Error: Invalid input\n");
                continue;
            }
            if (execute(&parser, NULL, code)) {
                // Error message already printed in execute
                continue;
            }
        }
    }
